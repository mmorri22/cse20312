...
class HashTable{
	...
	public:
		...
		// Must add a virtual destructor to Base Class
		virtual ~HashTable(){}
		...
		// Returns a valid key at a bucket location or throws an exception 
		Key& getKey( unsigned int location ){
			
			HashEntry temp = array.at( location );
			...
			// Cannot use temp since it has local scope
			return array.at( location ).key;			
		}
		
		// Returns a valid key at a bucket location or throws an exception 
		Value& getValue( unsigned int location ){
			...
			// Cannot use temp since it has local scope
			return array.at( location ).value;			
		}
