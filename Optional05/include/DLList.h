...
class DLList{
	...
	public:
		...
		* Post-conditions: int
		*
		* This function is added for Prob2.cpp
		* It returns the number of node in the list
		******************************************/
		int length(){
			node* curr = head;
			if (IsEmpty()){
		...
		******************************************/
		void removeFromList (int n){
			...
			if (n == 1){
				...
				// if the item wants to delete is in the middle part of the List
				for(int iter = 1; iter < n; ++iter){
					curr = curr->prev;
				}
				// the condition where head is wanted to be deleted after iteration
				if (curr == head){
					temp = curr;
					head = temp->next;
					head->prev = NULL;
