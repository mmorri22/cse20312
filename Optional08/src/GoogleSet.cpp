...
* 
* Compilation command: g++ -g -std=c++11 -Wpedantic GoogleSet.cpp -o GoogleSet
* Run: ./GoogleSet [DictionaryFile] [License]
* Example: ./GoogleSet words.txt RT011OS
...
* Pre-conditions : std::ifstream& dictionaryFile, std::set< std::string >& dictionarySet
* Post-conditions: none
*  
* This function reads all the dictionary words, and stores them in a STL Singly Linked Hash Table 
********************************************/
void getDictionaryWords(std::ifstream& dictionaryFile, std::set< std::string >& dictionarySet){
	// Get the strings and put in the Forward Hash Table
	std::string wordIn;
	while (dictionaryFile >> wordIn)
	{
		dictionarySet.insert( wordIn );
	}
}
...
void checkPlate(std::set< std::string > dictionarySet, char* plateStr){
	...
	for(std::set<std::string>::iterator iter = dictionarySet.begin(); iter != dictionarySet.end(); iter++){
		...
		if(iter->length() < finalWord.length() || finalWord.length() == 0)
			HashWord(HashPlate, *iter, finalWord);
	}
...
int main(int argc, char** argv){
	...
	// Get the strings and put in the Forward Hash Table
	getDictionaryWords(dictionaryFile, dictionarySet);
	...
	checkPlate(dictionarySet, (char*)"RCT100SA");
	checkPlate(dictionarySet, (char*)"RT123SO");
	checkPlate(dictionarySet, (char*)"AQ10S0K");
	checkPlate(dictionarySet, (char*)"TNT055RB");
	checkPlate(dictionarySet, (char*)"LET10EE0");
	checkPlate(dictionarySet, (char*)"AB1C1D1");
	checkPlate(dictionarySet, (char*)"AEI1O2U3");
	checkPlate(dictionarySet, (char*)"OTR1N2E3");
	checkPlate(dictionarySet, (char*)"AM1E2D3");
	checkPlate(dictionarySet, (char*)"SHR5I3I3");
