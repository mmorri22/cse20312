...
class SLList{
	...
	private:
		...
		/********************************************
		* Function Name  : findNthToLast
		* Pre-conditions : int nthToLast, node* curr, int& element 
		* Post-conditions: void
		* 
		* Private method which is the recursive solution 
		********************************************/			
		void findNthToLast(int nthToLast, node* curr, int& element) const{
			...
			// Recursive Case
				...
				findNthToLast(nthToLast, curr->next, element);
				element++;	
				
				if( element == nthToLast ){
					std::cout << nthToLast << "-to-last element is " << curr->data << std::endl;
					element++;
					return;
				}
	...
	public:
		...
		********************************************/
		...
		/********************************************
		* Function Name  : findNthToLast
		* Pre-conditions : int nthToLast 
		* Post-conditions: void
		* 
		* Public method which calls recursive solution 
		********************************************/		
		void findNthToLast(int nthToLast) const{
			
			int element = 0;
			
			findNthToLast( nthToLast, head, element );
			
			if(nthToLast > element){
				
				std::cout << "There are not " << nthToLast << " elements in the list." << std::endl;
			}
		}
