...
class BST{
	...
	protected:
		...
		BSTNode<T>* getNodeAtValue( BSTNode< T >* currPtr, const T& value ){
			...
			if( currPtr->data == value ){
				...
				return getNodeAtValue( currPtr->left, value );
				
			}
			else{
				
				return getNodeAtValue( currPtr->right, value );
			}
			
		}
		
		
		// Private Recursive Call for Inverting the Binary Search Tree
		void InvertTree(BSTNode< T >*& currPtr){
			...
			InvertTree( currPtr->left );
			
			InvertTree( currPtr->right );
	...
	public: 
		...
		BSTNode<T>* getNodeAtValue( const T& value ){
			
			
			return getNodeAtValue( root, value );
		...
		T kthSmallest( unsigned int kTh ){
			
			if( root == NULL ){
				
				COUT << "Empty tree. Can't find " << kTh << "th smallest." << ENDL;
