...
class BinomialHeap
{
  ...
  public:
	...
	// Copy Constructor
    BinomialHeap( const BinomialHeap<T>& rhs ) : theTrees( rhs.theTrees.size( ) ), currentSize( rhs.currentSize )
    { 
		// Go through each node 
        for( int i = 0; i < rhs.theTrees.size( ); ++i ){
		
			// Clone and add to the vectoe
            theTrees[ i ] = clone( rhs.theTrees[ i ] );
		}
    }

	// Assignment Operator
    BinomialHeap<T>& operator=( const BinomialHeap<T>& assign )
    {
		if( this != &assign ){
			
			// Resize the std::vector 
			this->theTrees.resize( assign.theTrees.size() );
			
			// Update the current size
			this->currentSize = assign.currentSize;
			
			// Copy the BinomialNodes in each element in the vector
			for( int i = 0; i < assign.theTrees.size( ); ++i ){
				this->theTrees[ i ] = this->clone( assign.theTrees[ i ] );
			}
		}
        return *this;
    }
	...
	friend std::ostream& operator<<( std::ostream& out, const BinomialHeap<T>& printHeap ){
		
		printHeap( out, printHeap[0] );
